...
cprequire_test(["inline:com-zipwhip-widget-svg2gcode-yg"], function(myWidget) {
...
cpdefine("inline:com-zipwhip-widget-svg2gcode-yg", ["chilipeppr_ready", "Snap", "Clipper" ], function() {
    return {
        /**
         * The ID of the widget. You must define this and make it unique.
         */
        id: "com-zipwhip-widget-svg2gcode-yg", // Make the id the same as the cpdefine id
        ...
        btnSetup: function() {
            ...
            $('#' + this.id + ' .input-servoup').change(this.generateGcode.bind(this));
            $('#' + this.id + ' .input-servodown').change(this.generateGcode.bind(this));
            $('#' + this.id + ' .input-interpolateresolution').change(this.generateGcode.bind(this));
        ...
        getSettings: function() {
            ...
            this.options["servoup"] = $('#' + this.id + ' .input-servoup').val();
            this.options["servodown"] = $('#' + this.id + ' .input-servodown').val();
            this.options["interpolateresolution"] = $('#' + this.id + ' .input-interpolateresolution').val();
...
/************loop for extracting coordination from svg to pts (world) coordination  ******/
            //load interpolate distance from option
            var interpolateLength = parseInt(this.options.interpolateresolution);
            ...
            txtGrp.traverse( function(child) {
                if (child.type == "Line") {
                    ...
                    for (i = 0; i < child.geometry.vertices.length; i++) {
                        ...
                        if (i == 0) {                            
                            // move to start point ideally I should use G0 however currently using only speed setting code
                            var pt0 = { x: worldPt.x , y: worldPt.y, write: false };
                            pts.push(pt0);
                            previousPt = worldPt;
                        }
                        else {
                            ...
                            var pt = { x: worldPt.x, y: worldPt.y, write: true };
                            pts.push(pt);
                            previousPt = worldPt;
                        }
                    }
            ...
            // update gcode size etcetra
            $('#' + this.id + " .gcode").val(gcode).prop('disabled', false);
            ...
            $('#' + this.id + " .gcode-size").text(parseInt(gcode.length / 1024) + "KB");
        ...
        getGcode: function(pts){
            ...
            for (var i in pts) {
                ...
                if (param.write & !this.penState.write) {
                    // Pen down
                    code += "M3 S" + servoDown + "\n";
                    // set machine status to write mode
                    this.penState.write = true;
                }
                else if (!param.write & this.penState.write) {
                    // Pen up
                    code += "M3 S" + servoUp + "\n";
                    // release write mode
                    this.penState.write = false;
